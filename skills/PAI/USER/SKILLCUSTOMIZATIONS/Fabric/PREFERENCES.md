...
When running the `extract_wisdom` pattern (via `/extract-wisdom` or any Fabric extract_wisdom invocation), **automatically save the output** to `~/SecondBrain/Knowledge/` after displaying it. This applies ONLY to extract_wisdom, not other Fabric patterns.
...
3. **Related section:** End with `## Related` containing `[[wiki-links]]` to related existing files in `~/SecondBrain/Knowledge/`.
