...
TOKEN_USAGE_LOG="$PAI_DIR/MEMORY/STATE/token-usage.jsonl"
TOKEN_CURRENT="$PAI_DIR/MEMORY/STATE/token-current.json"
...
# Cross-platform file mtime (returns epoch seconds)
...
# ─────────────────────────────────────────────────────────────────────────────
# TOKEN USAGE TRACKING - Write current session tokens for accumulation
# ─────────────────────────────────────────────────────────────────────────────
# Write current session snapshot (overwritten each statusline call, finalized at session end)
if [ "$total_input" -gt 0 ] || [ "$total_output" -gt 0 ]; then
    _today=$(date +%Y-%m-%d)
    _session_pid=$PPID
    printf '{"date":"%s","input":%d,"output":%d,"pid":%d,"ts":%d}\n' \
        "$_today" "$total_input" "$total_output" "$_session_pid" "$(date +%s)" \
        > "$TOKEN_CURRENT" 2>/dev/null
fi
...
# --- PARALLEL BLOCK START ---
{
    ...
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if [ -f "$GIT_REPO_CACHE" ]; then
            git_cache_mtime=$(file_mtime "$GIT_REPO_CACHE")
...

{
    ...
    cache_age=999999
    [ -f "$LOCATION_CACHE" ] && cache_age=$(($(date +%s) - $(file_mtime "$LOCATION_CACHE")))
...

{
    ...
    cache_age=999999
    [ -f "$WEATHER_CACHE" ] && cache_age=$(($(date +%s) - $(file_mtime "$WEATHER_CACHE")))
...
{
    # 5. Token usage - sum historical tokens by Today/This Month/Last Month
    _today=$(date +%Y-%m-%d)
    _this_month=$(date +%Y-%m)
    _last_month=$(date -d "$(date +%Y-%m-01) -1 day" +%Y-%m 2>/dev/null || date -v-1m +%Y-%m 2>/dev/null || echo "")

    tokens_today=0
    tokens_this_month=0
    tokens_last_month=0

    if [ -f "$TOKEN_USAGE_LOG" ] && [ -s "$TOKEN_USAGE_LOG" ]; then
        eval "$(jq -rs --arg today "$_today" --arg month "$_this_month" --arg lmonth "$_last_month" '
            (map(select(.date == $today)) | map(.input + .output) | add // 0) as $td |
            (map(select(.date | startswith($month))) | map(.input + .output) | add // 0) as $tm |
            (if $lmonth != "" then (map(select(.date | startswith($lmonth))) | map(.input + .output) | add // 0) else 0 end) as $lm |
            "tokens_today=\($td)\ntokens_this_month=\($tm)\ntokens_last_month=\($lm)"
        ' "$TOKEN_USAGE_LOG" 2>/dev/null)"
    fi

    # Add current live session tokens (not yet logged to JSONL)
    if [ -f "$TOKEN_CURRENT" ]; then
        _cur_input=$(jq -r '.input // 0' "$TOKEN_CURRENT" 2>/dev/null)
        _cur_output=$(jq -r '.output // 0' "$TOKEN_CURRENT" 2>/dev/null)
        _cur_date=$(jq -r '.date // ""' "$TOKEN_CURRENT" 2>/dev/null)
        _cur_total=$((_cur_input + _cur_output))

        # Add live session to today's count if it's today
        if [ "$_cur_date" = "$_today" ]; then
            tokens_today=$((tokens_today + _cur_total))
        fi
        # Add to this month's count if it's this month
        if [ "${_cur_date:0:7}" = "$_this_month" ]; then
            tokens_this_month=$((tokens_this_month + _cur_total))
        fi
    fi

    cat > "$_parallel_tmp/tokens.sh" << TOKEOF
tokens_today=${tokens_today:-0}
tokens_this_month=${tokens_this_month:-0}
tokens_last_month=${tokens_last_month:-0}
TOKEOF
} &
...
[ -f "$_parallel_tmp/tokens.sh" ] && source "$_parallel_tmp/tokens.sh"
...
# Token usage (amber/gold theme)
TOKEN_PRIMARY='\033[38;2;251;191;36m'     # Amber
TOKEN_LABEL='\033[38;2;245;158;11m'       # Darker amber for labels
TOKEN_VALUE='\033[38;2;252;211;77m'       # Gold for values
TOKEN_DIM='\033[38;2;180;140;60m'         # Muted gold for last month

# ─────────────────────────────────────────────────────────────────────────────
# HELPER FUNCTIONS
# ─────────────────────────────────────────────────────────────────────────────

# Format token count to human-readable (e.g., 1.2M, 450K, 12K)
format_tokens() {
    local count=$1
    if [ "$count" -ge 1000000 ]; then
        local m=$((count / 1000000))
        local r=$(( (count % 1000000) / 100000 ))
        if [ "$r" -gt 0 ]; then
            echo "${m}.${r}M"
        else
            echo "${m}M"
...
# ═══════════════════════════════════════════════════════════════════════════════
# LINE 2: TOKEN USAGE (Today / This Month / Last Month)
# ═══════════════════════════════════════════════════════════════════════════════

# Format values
_fmt_today=$(format_tokens "${tokens_today:-0}")
_fmt_month=$(format_tokens "${tokens_this_month:-0}")
_fmt_last=$(format_tokens "${tokens_last_month:-0}")
_fmt_session=$(format_tokens "$((total_input + total_output))")

case "$MODE" in
    nano)
        printf "${TOKEN_PRIMARY}⊘${RESET} ${TOKEN_VALUE}${_fmt_today}${RESET} ${SLATE_600}│${RESET} ${TOKEN_DIM}${_fmt_month}${RESET}\n"
        ;;
    micro)
        printf "${TOKEN_PRIMARY}⊘${RESET} ${TOKEN_LABEL}D:${RESET}${TOKEN_VALUE}${_fmt_today}${RESET} ${TOKEN_LABEL}M:${RESET}${TOKEN_VALUE}${_fmt_month}${RESET} ${TOKEN_DIM}L:${_fmt_last}${RESET}\n"
        ;;
    mini)
        printf "${TOKEN_PRIMARY}⊘${RESET} ${TOKEN_LABEL}TOKENS:${RESET} "
        printf "${TOKEN_LABEL}Ses:${RESET} ${TOKEN_VALUE}${_fmt_session}${RESET} "
        printf "${SLATE_600}│${RESET} ${TOKEN_LABEL}Day:${RESET} ${TOKEN_VALUE}${_fmt_today}${RESET} "
        printf "${SLATE_600}│${RESET} ${TOKEN_LABEL}Mo:${RESET} ${TOKEN_VALUE}${_fmt_month}${RESET} "
        printf "${SLATE_600}│${RESET} ${TOKEN_DIM}Last:${RESET} ${TOKEN_DIM}${_fmt_last}${RESET}\n"
        ;;
    normal)
        printf "${TOKEN_PRIMARY}⊘${RESET} ${TOKEN_LABEL}TOKENS:${RESET} "
        printf "${TOKEN_LABEL}Session:${RESET} ${TOKEN_VALUE}${_fmt_session}${RESET} "
        printf "${SLATE_600}│${RESET} ${TOKEN_LABEL}Today:${RESET} ${TOKEN_VALUE}${_fmt_today}${RESET} "
        printf "${SLATE_600}│${RESET} ${TOKEN_LABEL}This Month:${RESET} ${TOKEN_VALUE}${_fmt_month}${RESET} "
        printf "${SLATE_600}│${RESET} ${TOKEN_DIM}Last Month:${RESET} ${TOKEN_DIM}${_fmt_last}${RESET}\n"
        ;;
esac
printf "${SLATE_600}────────────────────────────────────────────────────────────────────────${RESET}\n"
...
if [ -f "$RATINGS_FILE" ] && [ -s "$RATINGS_FILE" ]; then
    ...
    if [ -f "$LEARNING_CACHE" ]; then
        cache_mtime=$(file_mtime "$LEARNING_CACHE")
        ratings_mtime=$(file_mtime "$RATINGS_FILE")
...
if [ "$MODE" = "normal" ]; then
    ...
    quote_age=$(($(date +%s) - $(file_mtime "$QUOTE_CACHE")))
